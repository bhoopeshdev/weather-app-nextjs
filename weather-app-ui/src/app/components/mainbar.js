    ...
    return (
        <div className="h-80 flex flex-col bg-white rounded-3xl">
            <div className="w-full px-8 mt-8 flex flex-row gap-4">
                <p className="text-xl font-bold text-gray-700">Today</p>
            </div>
            <div className="w-full flex-1 flex flex-row justify-start mx-4 mb-2 gap-4 overscroll-auto overflow-auto">
                {forecast.forecastday[0].hour.map((data, index) => {
                    return (
                        <div className="h-full flex flex-col items-center justify-center w-36 shrink-0" key={index}>
                            ...
                            <p className="text-xs text-gray-700">{formatTo12HourTime(data.time)}</p>
                            <p className="text-xs text-gray-700 degree-symbol">{data.temp_c}</p>
                        </div>
                    )
                })}
            </div>
